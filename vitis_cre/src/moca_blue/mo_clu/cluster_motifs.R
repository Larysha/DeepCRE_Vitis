...
# Defaults - these should match the output from moca_blue/mo_nom/get_matrices.R 
default_spec <- "vitis"
default_model <- "ssr"
default_input_dir <- "results/moca_blue/mo_nom"
default_output_dir <- "results/moca_blue/mo_clu"
...
if (length(args) >= 4 && nzchar(args[4])) {
  ...
  # e.g., mo_nom/rdf5_vitis_ssr_cwm-motifs.jaspar (the seqlet normalised version)
  pattern <- paste0("rdf5_.*", SPEC, MODEL, "_cwm-motifs\\.jaspar$")
  jaspar_files <- list.files(default_input_dir, pattern = pattern, full.names = TRUE)
  
  if (length(jaspar_files) == 0) {
    stop("No CWM JASPAR files found matching pattern: ", pattern,
