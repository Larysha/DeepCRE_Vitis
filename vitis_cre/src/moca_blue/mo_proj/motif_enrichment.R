...
source("moca_blue/utils.R")

# Command line arguments with defaults
args <- commandArgs(trailingOnly = TRUE)

# Default file paths for Vitis analysis
default_motif_file <- "results/moca_blue/mo_proj/20250825_vitis_ssr_q10q90_filtered.csv"
default_prediction_file <- "results/predictions/vitis_02_SSR_deepcre_predict_250805_220713.csv"
default_expression_file <- "tpm_counts/vitis_drought_leaf_targets.csv"
default_output_dir <- "results/moca_blue/mo_proj"
...
load_enrichment_data <- function(motif_file, prediction_file, expression_file) {
  ...
  # Load model predictions with enhanced validation logic
  ...
  if (file.exists(prediction_file)) {
    ...
    colnames(prediction_data) <- c("true_targets", "pred_probs", "gene_id")
    pred_available <- TRUE
    cat("  Loaded", nrow(prediction_data), "prediction results\n")
    cat("  Prediction file state: AVAILABLE\n")
  } else {
    cat("  Warning: Prediction file not found, creating dummy binary predictions\n")
    cat("  Prediction file state: MISSING - using dummy data\n")
    
    # Create dummy binary predictions (0 or 1) for genes in motif data
    # This mimics the original V1.7 logic for missing prediction files
    unique_genes <- unique(motif_data$gene_id)
    prediction_data <- data.frame(
      gene_id = unique_genes,
      # Binary predictions (0 = low, 1 = high) - more realistic than continuous
      pred_probs = sample(c(0, 1), length(unique_genes), replace = TRUE, prob = c(0.6, 0.4)),
      true_targets = sample(c(0, 1, 2), length(unique_genes), replace = TRUE, prob = c(0.4, 0.4, 0.2))
    )
    cat("  Generated", length(unique_genes), "dummy predictions (60% low, 40% high)\n")
  }
  ...
  expression_data <- read.csv(expression_file, stringsAsFactors = FALSE)
  # Standardize column names
  colnames(expression_data) <- c("gene_id", "target")
  cat("  Loaded", nrow(expression_data), "expression targets\n")
...
integrate_enrichment_data <- function(data_list) {
  ...
  # Filter out medium expression genes (focus on low vs high)
