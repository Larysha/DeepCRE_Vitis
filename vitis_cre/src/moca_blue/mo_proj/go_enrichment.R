...
# Default file paths
default_motif_performance <- "results/moca_blue/mo_proj/20250825_vitis_motif_performance_motif_performance.csv"
default_integrated_data <- "results/moca_blue/mo_proj/20250825_vitis_motif_performance_integrated_data.csv"  # Need to save this from main script
default_go_file <- "gene_ontology/V_vinifera_ont_converted.gmt"  # Use converted file
...
generate_go_enrichment_plot <- function(go_enrichment, performance_metrics, output_dir) {
  
  cat("Generating GO enrichment plot...\n")
  
  # Filter to forward strand motifs only (remove _R_ reverse strand motifs)
  all_motifs <- performance_metrics %>%
    filter(!grepl("_R_", epm)) %>%  # Keep only forward strand (_F_) motifs
    pull(epm)
  
  cat("  Including", length(all_motifs), "forward strand motifs in analysis\n")
  ...
  if (nrow(go_plot_data) > 0) {
    ...
    if (nrow(go_detailed_with_hierarchy) > 0) {
      ...
      title_text <- "Gene Ontology (GO) Enrichment"
      subtitle_text <- paste0("Filtering: ≥50% motif coverage, max enrichment ≥0.003, showing last 2 GO term levels\n",
                             ...
                             "Analysis of ", length(all_motifs), " forward strand motifs")
      
      # Save the heatmap
      pdf_file <- file.path(output_dir, paste0(DATE_STAMP, "_", PROJECT_NAME, "_go_enrichment.pdf"))
      ...
      cat("  Saved detailed GO enrichment heatmap with Level 1 gradient annotation\n")
      return(ht)
    } else {
      cat("  Warning: No detailed GO categories with sufficient data across motifs\n")
      return(NULL)
    }
  } else {
    cat("  Warning: No data available for GO enrichment plot\n")
    return(NULL)
  }
}
...
go_plot <- generate_go_enrichment_plot(go_enrichment, analysis_results$performance, OUTPUT_DIR)
