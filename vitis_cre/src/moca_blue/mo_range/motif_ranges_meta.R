...
default_input_dir <- "results/moca_blue/mo_range"
default_output_dir <- "results/moca_blue/mo_range"
...
if (length(args) >= 4 && nzchar(args[4])) {
  ...
  cat("Searching for HDF5 source file in results/modisco/...\n")
  source_file_path <- find_project_files(
    directory = "results/modisco",
    pattern = ".*\\.hdf5$",
