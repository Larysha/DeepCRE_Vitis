# This script processes MoDisco HDF5 files to extract motif matrices (PFM, PWM, CWM) in JASPAR format
...
# handles PFMs/PWMs from sequence field with adjusted background frequencies
# DeepCRE/moca_blue/mo_nom
######################
...
default_output_dir <- "results/moca_blue/mo_nom"
...
if (!dir.exists("results/moca_blue")) dir.create("results/moca_blue", recursive = TRUE)
...
pwm_named <- apply_nomenclature(pwm_processed, seqlet_counts)
cwm_named <- apply_nomenclature(cwm_processed, seqlet_counts)

# *** NOTE ON CWM SCALING ***
# The original script applied: abs(matrix) * seqlet_count / max(abs(matrix))
# BUT this was commented out in the final version
# up to now, this revised script preserves raw contribution scores which are interpretable:
# — maintains regulatory directionality (positive = activating, negative = repressing)
# however if we want to add this scaling back in, we can uncomment the section below.


####
# UPDATE:
#####
# It is neccessary to apply the original CWM scaling logic for the motif clustering step
...
# recommend running this script twice: once with the original CWM scaling logic applied
# and once without, to compare results (just rename files so they don't overwrite) 

# CWMs chosen over PWMs for clustering since CWMs represent the contribution of each motif to expression
# the values are tailored to prioritise patterns with stronger "evidence" (more seqlets)
# PWMs log-odds scores represent binding affinity (general applicability) - but not task specific contribution
# they also depend on backgound frequencies which can vary by species
# in summary, CWMs reflect what the deep learning model actually learned, not just sequence preferences

#######################################################
# Apply original CWM scaling logic (post-nomenclature)
#######################################################

apply_original_cwm_scaling <- function(named_matrices) {
  # Apply the original CWM scaling logic after nomenclature
  ...
  for (i in seq_along(all_matrices)) {
    ...
    # Apply original transformation: abs(matrix) * seqlet_count / max(abs(matrix))
...
cwm_named <- apply_original_cwm_scaling(cwm_named)
...
summarise_contrib_scores <- function(matrices, matrix_type = "CWM") {
  ...
  imap_dfr(all_matrices, function(mat, motif_name) {
    ...
    } else if (matrix_type == "PWM") {
      # For PWMs, we summarise information content
      tibble(
        motif = motif_name,
        matrix_type = matrix_type,
        score_sum = sum(mat),  # Sum of log-odds
        score_max = max(mat),
        score_min = min(mat),
        score_range = max(mat) - min(mat)
      )
...
cwm_summary <- summarise_contrib_scores(cwm_named, "CWM")
pwm_summary <- summarise_contrib_scores(pwm_named, "PWM") 
pfm_summary <- summarise_contrib_scores(pfm_named, "PFM")

# Combine all summaries
combined_summary <- bind_rows(cwm_summary, pwm_summary, pfm_summary)
...
write_jaspar_motifs <- function(motif_matrices, out_file, matrix_type = "motif") {
  ...
  for (motif_name in names(all_matrices)) {
    ...
    for (j in 1:nrow(mat)) {
      if (matrix_type == "PWM") {
        # Format PWM values to reasonable precision
        values <- sprintf("%.4f", mat[j, ])
      } else {
...
pwm_summary_file <- file.path(dirpath_out, paste0(DATE, "_", SPEC, MODEL, "_pwm_summary.csv"))
...
write.csv(pwm_summary, file = pwm_summary_file, row.names = FALSE)
write.csv(pfm_summary, file = pfm_summary_file, row.names = FALSE)

cat("CWM summary statistics written to:", cwm_summary_file, "\n")
cat("PWM summary statistics written to:", pwm_summary_file, "\n")
cat("PFM summary statistics written to:", pfm_summary_file, "\n")

# JASPAR format files with original naming convention
pfm_jaspar <- file.path("results/moca_blue", paste0("rdf5_PFM_pattern", SPEC, MODEL, "_pfm-motifs.jaspar"))
pwm_jaspar <- file.path("results/moca_blue", paste0("rdf5_epm", SPEC, MODEL, "_pwm-motifs.jaspar"))
cwm_jaspar <- file.path("results/moca_blue", paste0("rdf5_epm", SPEC, MODEL, "_cwm-motifs.jaspar"))

write_jaspar_motifs(pfm_named, pfm_jaspar, "PFM")
write_jaspar_motifs(pwm_named, pwm_jaspar, "PWM")
write_jaspar_motifs(cwm_named, cwm_jaspar, "CWM")

cat("PFM motifs written to:", pfm_jaspar, "\n")
cat("PWM motifs written to:", pwm_jaspar, "\n")
cat("CWM motifs written to:", cwm_jaspar, "\n")

# Close HDF5 file
H5Fclose(h5file)

cat("Processing complete!\n")
cat("Found", nrow(cwm_summary), "CWM,", nrow(pwm_summary), "PWM, and", nrow(pfm_summary), "PFM motifs\n")
cat("Use PWM file for BLAMM scanning, CWM file for interpretation\n")