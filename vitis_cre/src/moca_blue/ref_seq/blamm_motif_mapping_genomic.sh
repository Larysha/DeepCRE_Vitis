...
# Look for PFM file first (preferred), then fall back to PWM
MOTIFS_FILE=""
if ! MOTIFS_FILE=$(find_file "rdf5_PFM_patternvitisssr_pfm-motifs.jaspar" "${MOTIFS_DIRS[@]}"); then
    echo "Warning: Could not find PFM motifs file, trying PWM version..." >&2
    if ! MOTIFS_FILE=$(find_file "rdf5_epmvitisssr_pwm-motifs.jaspar" "${MOTIFS_DIRS[@]}"); then
        echo "Error: Could not find motifs file (PFM or PWM) in directories: ${MOTIFS_DIRS[*]}" >&2
        echo "Expected files: rdf5_PFM_patternvitisssr_pfm-motifs.jaspar (preferred) or rdf5_epmvitisssr_pwm-motifs.jaspar" >&2
        exit 1
    fi
fi
...
echo "Found motifs file (PFM format preferred): $MOTIFS_FILE"
...
echo "Motifs file (PFM format preferred): $MOTIFS_FILE"
