...
h5_file <- if (length(args) >= 1 && !is.na(args[1])) {
  ...
  h5_files <- list.files("results/shap", pattern = "\\.h5$", full.names = TRUE)
  if (length(h5_files) == 0) stop("No .h5 files found in results/shap/")
...
csv_file <- if (length(args) >= 2 && !is.na(args[2])) {
  ...
  csv_files <- list.files("results/predictions", pattern = "\\.csv$", full.names = TRUE)
  if (length(csv_files) == 0) stop("No .csv files found in results/predictions/")
...
shap_meta_file <- if (length(args) >= 3 && !is.na(args[3])) {
  ...
  meta_files <- list.files("results/shap", pattern = "shap_meta\\.csv$", full.names = TRUE)
  if (length(meta_files) == 0) stop("No shap_meta.csv files found in results/shap/")
  if (length(meta_files) > 1) {
    cat("Multiple shap_meta.csv files found, using:", meta_files[1], "\n")
  }
  meta_files[1]
} 
output_dir_plots <- if (length(args) >= 4) args[4] else "results/moca_blue/mo_imp/figures/saliency_top5"
