...
def modisco_run(contribution_scores: np.ndarray, hypothetical_scores: np.ndarray, one_hots: np.ndarray, output_name: str):
    ...
    """
    folder_path = make_absolute_path('../../out', 'modisco', start_file=__file__)
    ...
    # adjustments made from original: smaller sliding window, less stringent FDR, halved min cluster size
        ...
        seqlets_to_patterns_factory=modisco.tfmodisco_workflow.seqlets_to_patterns.TfModiscoSeqletsToPatternsFactory(
            ...
            n_cores=5 # controls parallel Leiden clustering (~12Gb per job - 12x 5 cores = 60 + GB - changed to 2)
        )
    )(
        task_names=['task0'],
...
def generate_motifs(genome_file: str, annotation_file: str, tpm_targets_file: str, extragenic: int, intragenic: int, ignore_small_genes: bool,
    ...
    if not force_interpretation:
        try: 
            ...
            print(os.getcwd())
            print(os.path.join("../../out", "shap"))
            saved_interpretation_results_path = find_newest_interpretation_results(output_name=output_name, results_path=os.path.join("../../out", "shap"))
