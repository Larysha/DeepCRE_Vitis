...
def set_up_train_val_split_variables(annotation: pd.DataFrame, validation_genes_count: int, validation_fraction: float = 0.2) -> Tuple[int, int]:
    """sets up the variables for the train val split - uses all available validation genes and removes training size limits

    Args:
        annotation (pd.DataFrame): DataFrame containing the gene annotations
        validation_genes_count (int): number of genes available for validation (from pickle file)
    ...
    # Use all available validation genes instead of fixed percentage
    target_val_size = validation_genes_count  # Use all validation genes from pickle
    target_train_size = total_sequences       # Remove training size limit to include all chromosomes
    
    actual_val_ratio = (validation_genes_count / total_sequences) * 100
    actual_train_ratio = ((total_sequences - validation_genes_count) / total_sequences) * 100
    
    print(f"=== TRAIN/VAL SPLIT CONFIGURATION ===")
    print(f"Total genes: {total_sequences}")
    print(f"Validation genes available: {validation_genes_count} ({actual_val_ratio:.1f}%)")
    print(f"Training genes target: {total_sequences - validation_genes_count} ({actual_train_ratio:.1f}%)")
    print(f"NOTE: Removed training size limit to ensure all chromosomes (especially chr19) are included")
    print("=====================================")
    
    return target_val_size, target_train_size


def save_skipped_genes(skipped_genes: List[str], time_stamp: str, skipped_reasons: dict = None, selection_details: dict = None) -> None:
    """saves the skipped genes to a file with detailed reasons and selection logic

    Args:
        skipped_genes (List[str]): list containing the gene ids that were skipped
        time_stamp (str): time stamp of the current run
        skipped_reasons (dict, optional): dictionary mapping gene_id to skip reason
        selection_details (dict, optional): dictionary mapping gene_id to selection logic details
    """
    if skipped_genes:  # This checks if the set/list is not empty
        file_name = f'skipped_genes_{time_stamp}.txt'
        file_name = make_absolute_path("../../out", "training", file_name, start_file=__file__)
        with open(file_name, 'w') as skipped_genes_file:
            skipped_genes_file.write("Gene_ID\tReason\tSelection_Details\n")  # Header
            for gene in skipped_genes:
                reason = skipped_reasons.get(gene, "Unknown") if skipped_reasons else "Unknown"
                details = selection_details.get(gene, "N/A") if selection_details else "N/A"
                skipped_genes_file.write(f"{gene}\t{reason}\t{details}\n")
...
def extract_genes_training(genome_path: str, annotation_path: str, tpm_path: str, extragenic: int, intragenic: int, genes_pickled: str,
    ...
    target_val_size, target_train_size = set_up_train_val_split_variables(annotation=annotation, validation_genes_count=len(validation_genes), validation_fraction=validation_fraction)
        
    train_seqs, val_seqs, train_targets, val_targets = [], [], [], []
    skipped_genes = []
    skipped_reasons = {}  # Track detailed skip reasons
    selection_details = {}  # Track selection logic details for skipped genes file
    ...
    for specie, chrom, start, end, strand, gene_id in annotation.values:
        if gene_id not in tpms.index:
            ...
            skipped_reasons[gene_id] = "Missing_TPM_data"
            continue
        ...
        if added_train == 0 and added_val == 0:
            skipped_genes.append(gene_id)
            if seq.shape[0] != expected_final_size:
                skipped_reasons[gene_id] = f"Wrong_sequence_length_{seq.shape[0]}_expected_{expected_final_size}"
            elif not (include_in_validation_set or include_in_training_set):
                skipped_reasons[gene_id] = "Not_selected_for_training_or_validation"
            else:
                skipped_reasons[gene_id] = "Unknown_reason"
            
            # Store selection details for skipped genes file
            selection_details[gene_id] = f"include_val={include_in_validation_set},include_train={include_in_training_set},val_size={current_val_size}/{target_val_size},train_size={current_train_size}/{target_train_size},in_validation_genes={gene_id in validation_genes}"
    ...
    save_skipped_genes(skipped_genes, time_stamp=time_stamp, skipped_reasons=skipped_reasons, selection_details=selection_details)
    ...
    # Calculate and report actual train/val ratios achieved
    actual_train_count = len(train_seqs)
    actual_val_count = len(val_seqs)
    total_used = actual_train_count + actual_val_count
    
    actual_train_ratio = (actual_train_count / total_used) * 100 if total_used > 0 else 0
    actual_val_ratio = (actual_val_count / total_used) * 100 if total_used > 0 else 0
    
    print(f"\n=== FINAL TRAIN/VAL SPLIT ACHIEVED ===")
    print(f"Training genes used: {actual_train_count} ({actual_train_ratio:.1f}%)")
    print(f"Validation genes used: {actual_val_count} ({actual_val_ratio:.1f}%)")
    print(f"Total genes used: {total_used}")
    print(f"Total genes skipped: {len(skipped_genes)}")
    print("======================================")
    
    print(f"Array shapes - Train: {train_seqs.shape}, Validation: {val_seqs.shape}")
