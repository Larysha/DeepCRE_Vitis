...
def find_newest_interpretation_results(output_name: str, results_path: str = "") -> str:
    ...
    """
    if results_path == "":
        path_to_interpretations = make_absolute_path("../../out", "shap", start_file=__file__)
    else:
...
def compute_actual_hypothetical_scores(x, model) -> Tuple[np.ndarray, np.ndarray]:
    ...
    dinuc_shuff_explainer = shap.DeepExplainer(
        (model.input, model.layers[-2].output[:, 0]), # model input/output
        data=dinuc_shuffle_several_times,   # background generator
        combine_mult_and_diffref=combine_mult_and_diffref)  # score calculation
...
def extract_scores(genome_file_name: str, annotation_file_name: str, tpm_counts_file_name: str, extragenic: int,
    ...
    """
    folder_path = make_absolute_path("../../out", "shap")
...
def save_results(output_name: str, shap_actual_scores: np.ndarray, shap_hypothetical_scores: np.ndarray,
    ...
    """
    folder_name = make_absolute_path("../../out", "shap")
    ...
    with h5py.File(name=h5_file_name, mode='w') as h5_file:
        ...
        save_path = make_absolute_path('../../out', 'shap', f'{output_name}_{file_name}_{get_time_stamp()}_shap_meta.csv', start_file=__file__)
